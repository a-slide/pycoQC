...
pycoQC needs a text summary file generated by ONT Albacore. For 1D run use the file named sequencing_summary.txt available the root of Albacore output directory. For 1D2, use sequencing_1dsq_summary.txt that cam be found in the 1dsq_analysis directory.
...
The Jupyter Notebook is a fantastic tool that can be used in many different ways, in particular to present you analyses in an interactive environment to other people.

One of the specificity of pycoQC is to have a rich python API meant to be used directly inside a Jupyter notebook. The pycoQC API for Jupyter is very flexible and allows you to explore the your nanopore data interactively and more in depth than with the command line interface
...
If it does not auto-start, open manually the following URL in you favorite web browser http://localhost:8888/tree
...
```
usage: pycoQC [-h] [--version] [--file FILE] [--outfile OUTFILE]
