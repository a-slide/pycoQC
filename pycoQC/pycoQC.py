...
class pycoQC():
    ...
    #~~~~~~~FUNDAMENTAL METHODS~~~~~~~#
    def __init__ (self, seq_summary_file, run_type="1D", runid=None, filter_zero_len=False, verbose=False):
        """
        Parse Albacore sequencing_summary.txt file and clean-up the data
        * seq_summary_file
            Path to the sequencing_summary.txt generated by Albacore
        * run_type
            Type of the run 1D, 1D2 or 2D (currently 2D and 1D2 run type are processed similarly) [Default 1D]
        * runid
            If you want a specific runid to be analysed. By default it will analyse all the reads in the file irrespective of their runid 
            [Default None]
        * filter_zero_len
        ...
        * verbose
        ...
        if run_type in ["1D", "1d"]:
            self.run_type = "1D"
            colnames = ["read_id", "run_id", "channel", "start_time", "num_events","sequence_length_template", "mean_qscore_template"]
            colrename = {"sequence_length_template":"num_bases", "mean_qscore_template":"mean_qscore"}
        elif run_type in ["1D2", "1d2", "2D", "2d"]:
            self.run_type = "2D"
            colnames = ["read_id", "run_id", "channel", "start_time", "sequence_length_2d", "mean_qscore_2d"]
            colrename = {"sequence_length_2d":"num_bases", "mean_qscore_2d":"mean_qscore"}
        else:
            raise ValueError("Invalid run_type 1D, 2D or 1D2")

        # Verify the presence of the columns required for pycoQC
        if verbose: jprint("Checking fields in dataframe", bold=True)        
        for col in colnames:
            if not col in self.df.columns:
                raise ValueError("Column {} not found in the provided sequence_summary file".format(col))
        
        if verbose: jprint("\tAll valid for run type {}".format(self.run_type))
        
        # Drop unused fields and simplify field names
        self.df = self.df[colnames]
        self.df.rename(columns=colrename, inplace = True)
        ...
        if runid:
            if verbose: jprint ("Selecting reads with Run_ID {}".format(runid), bold=True)
            l = len(self.df)
            self.df = self.df[(self.df["run_id"] == runid)]
            if verbose: jprint ("\t{} reads discarded".format(l-len(self.df)))
        
        # Reads per runid
        if verbose: jprint("Counting reads per runid", bold=True)
        self.runid_counts = self.df["run_id"].value_counts(sort=True).to_frame(name="Counts")
        if verbose: jprint("\tFound {} runid".format(len(self.runid_counts)))
        
        # Make a final cleanup
        if verbose: jprint("Final data cleanup", bold=True)
    ...
    def overview (self):
        ...
        jprint ("General counts", bold=True)
        ...
        jprint ("Read count per Run ID", bold=True)
        display(self.runid_counts)
        
        # Display distribution table
        jprint ("Distribution of quality scores and read lengths", bold=True)
        ...
        # Sample if needed
        df = self.df[["mean_qscore", "num_bases"]]
        if len(df) > 50000: 
            df = df.sample(50000)
            
        # Plot distributions
        fig, (ax1, ax2) = pl.subplots(1, 2, figsize=(12, 6))
        g1 = sns.violinplot(data=df["mean_qscore"], color="orangered", alpha=0.5, bw=.2, linewidth=1,
            inner="quartile", ax=ax1)
        t = ax1.set_title("Quality score distribution")
        g2 = sns.violinplot(data=df["num_bases"], color="orangered", alpha=0.5, bw=.2, cut=1, linewidth=1,
    ...
    def channels_activity (self, level="reads", figsize=[24,12], cmap="OrRd", alpha=1, robust=True, annot=True, fmt="d", cbar=False,
        ...
        * cbar
        ...
        if level == "events":
            if self.run_type == "2D":
                jprint ("events level is not available for 2D and 1D2 run types")
                return
    ...
    def output_over_time (self, level="reads", figsize=[30,7], color="orangered", alpha=0.5, win_size=0.25, **kwargs):
        """
