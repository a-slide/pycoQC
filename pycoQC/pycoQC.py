...
class pycoQC():
    ...
    #~~~~~~~FUNDAMENTAL METHODS~~~~~~~#
    def __init__ (self, seq_summary_file, run_type="", runids=[], filter_zero_len=False, filter_fail=False, verbose=False, **kwargs):
        """
        ...
        * filter_fail
        ...
        opt_col = self._check_columns (df=self.df, colnames=required_colnames, raise_error_if_missing=False)
        ...
        if filter_fail:
            if not "passes_filtering" in self.df:
                if verbose: jprint ("Cannot perform filtering as the information is not available", bold = True)
            else;
                if verbose: jprint ("Filter out zero length reads", bold = True)
                l = len(self.df)
                self.df = self.df[(self.df["passes_filtering"] > 0)]
        ...
        ################################################################################################################
        # Select runids is needed + Order per time per runid group + correct time = Increment by previous runid
        ################################################################################################################
        # Select and reorder runids if required
        if runid:
        ...
        ################################################################################################################
        # Could collect more info like time per runid, mean qual... Or it could be a specific function as well
        ################################################################################################################
        # Reads per runid
        ...
        self.df = self.df.reset_index(drop=True) ############### Needed ?
