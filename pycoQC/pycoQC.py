...
class pycoQC():
    ...
    def __init__ (self,
        ...
        * filter_calibration: BOOL [Default False]
        ...
        if run_type == "1D" or (not run_type and "sequence_length_template" in self.df):
            ...
            optional_colnames = ["num_events", "calibration_strand_genome_template", "passes_filtering", "barcode_arrangement"]
        ...
        # Drop unused fields and simplify field names and drop lines containing NA values
    ...
    def barcode_counts (self,
        include_fail_count = True,
        include_calibration_count = True,
        include_unclassified_count = True,
        min_percent_barcode = 1,
        figsize = 10,
        cmap = "Set2",
        sample = 100000,
        plot_style = "ggplot",
        **kwargs):
        """
        Plot pie plot of barcode distribution
        * include_fail_count: BOOL [Default True]
            If True fail quality reads are included in the figure
        * include_calibration_count: BOOL [Default True]
            If True calibration reads are included in the figure
        * include_unclassified_count: BOOL [Default True]
            If True unclassified reads (not attributed to a specific barcode) are included in the figure
        * min_percent_barcode INT [Default = 1]
            If a barcode is found in less this percentage of reads, it will be counted as unclassified
        * figsize: INT [Default 12]
            Size of square ploting area
        * cmap [Default "Set2"]
            Matplotlib colormap code to color the space (https://matplotlib.org/users/colormaps.html)
        ...
        * plot_style: STR [default "ggplot"]
        ...
        assert "barcode_arrangement" in self.df, "No barcode information available"

        # Filter out reads out of select boundaries
        df = self.df[["passes_filtering", "calibration_strand_genome_template", "barcode_arrangement"]]
        if sample and len(df) > sample:
            df = df.sample(sample)

        # Get Pass counts
        results_df = df[(df["passes_filtering"] == True) & (df["calibration_strand_genome_template"].isin(["filtered_out", "no_match"]))]
        count_series = results_df["barcode_arrangement"].value_counts()

        # Select only reads with given barcodes
        count_series_2 = pd.Series(data=[0], index=["Unclassified"])
        cuttoff = count_series.sum() * min_percent_barcode / 100
        for k, v in count_series.items():
            if k == "unclassified" or v < cuttoff:
                count_series_2["Unclassified"] += v
            else:
                count_series_2[k.title()] = v
        count_series = count_series_2

        # Remove unclassified reads
        if not include_unclassified_count:
            count_series_2 = pd.Series()
            for k, v in count_series.items():
                if not k == "Unclassified":
                    count_series_2[k] = v
            count_series = count_series_2

        # Add overall fail read counts
        if include_fail_count:
            n_fail = len(df[(df["passes_filtering"] == False) & (df["calibration_strand_genome_template"].isin(["filtered_out", "no_match"]))])
            count_series["Quality_fail"] = n_fail

        # Add overall calibration read counts
        if include_calibration_count:
            n_calibration = len(df[(~df["calibration_strand_genome_template"].isin(["filtered_out", "no_match"]))])
            count_series["Calibration"] = n_calibration

        # Plot
        with pl.style.context(plot_style):
            fig, ax = pl.subplots (figsize= (figsize, figsize))
            _ = count_series.plot.pie (ax=ax, autopct = self._make_autopct(count_series.values), colormap=cmap)
            _ = ax.set_title ("Distribution of reads per barcode")
            _ = ax.set_ylabel("")
